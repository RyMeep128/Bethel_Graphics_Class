...
void main() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        // === Toon-style light term (adapted from reference shadow shader) ===
        // Using normalized N·L, so this is in [-1, 1].
        float NdotL = dot(normal, L);

        // Reference uses 1.5/1.7 with non-normalized lightPos; here we pick a
        // more sane range for normalized NdotL. Tune these if needed.
        float toon = smoothstep(0.0, 0.2, NdotL);

        // Distance attenuation (1/r^2), like their (lightIntensity / (r*r))
        float r     = length(uLightPos[i].xyz - position);
        float inv_r2 = 1.0 / max(r * r, 0.0001);

        // We only need a scalar mask; using length of light color as intensity proxy
    ...
    // Watercolor-style alpha (shadow pass reference: no extra 0.9 factor)
    float alpha = (1.0 - (color.r * 0.8 +
    color.g * 0.1 +
    color.b * 0.7));

    watercolorShadowTex = vec4(color, alpha);
}
